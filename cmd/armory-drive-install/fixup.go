...
func clearOTAPublicKey(buf []byte) []byte {
	if !bytes.Contains(buf, assets.OTAPublicKey) {
		log.Fatal("could not locate OTA public key")
	}

	buf = bytes.ReplaceAll(buf, assets.OTAPublicKey, make([]byte, len(assets.OTAPublicKey)))
